<script setup name="SubscribeGroups">
import { ref } from 'vue'
...
const subscribeGroups = () => {
  groupRef.value.validate(async (valid) => {
    if (valid) {
      ...
      loading.vue = true
      let result = await getInstanceFCC().subscribeGroups(groupForm.value)
      setLog({
        name: '订阅群组结果',
        msg: result
      })
...
<el-form :model="groupForm" ref="groupRef" :rules="rules">
  <el-row>
    ...
    <el-col class="col-right" :span="8">
      <el-form-item>
        <el-button type="primary" @click="subscribeGroups" v-loading="loading">订阅群组</el-button>
      </el-form-item>
    </el-col>
  </el-row>
</el-form>
